        ...
        def next(term:Term):
            ...
            if abstract:
                if isinstance(term,AEnc):
                    # sl = map(Attacker.inst_term(dict),term.symbols)
                    # sl = [Attacker.inst_term(dict)(x) for x in term.symbols]
                    # print(f'term.symbols is {term.symbols}')
                    sl = Attacker.inst_term(term_dict)(term.symbols) 
                    # print(f'sl here is{sl}')
                    if term.key.is_abstract():
                        # k = deepcopy(term_dict["PKey"])
                        k = term_dict["PKey"]
                    else:
                        # k = deepcopy(term.key)
                        k = term.key
                    ...
                    # return deepcopy(term_dict[term.get_ttyp()])
                    if isinstance(term,list):
                        # return list(map(lambda x:deepcopy(term_dict[x.get_ttyp()]),term))
                        # return [(deepcopy(term_dict[x.get_ttyp()]) if x.is_abstract() else deepcopy(x)) for x in term]
                ...
                return term 
                # if isinstance(term,list):
                #     # ret = list(map(lambda x:deepcopy(x),term))
                #     ret = [deepcopy(x) for x in term]
                #     # print(f'ret is {ret}')
                #     return ret
                # else: 
